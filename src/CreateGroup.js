...
export default class CreateGroup extends React.Component {
  ...
  createGroup = () => {
    ...
    fetch(
      `http://localhost:8080/Chat/creategroup?username=${username}&friends=${memberNames}&gName=${groupName}`,
    )
      .then(res => res.json())
      .then(data => {
        console.log(data);
    ...
    return (
      <View>
        ...
        <Text style={styles.fieldText}>
          Enter name of friends to add separated by commas:{' '}
        </Text>
        <TextInput
          style={styles.fieldText}
          placeholder="Enter name of friends to add separated by commas:"
          value={memberNames}
          autoCapitalize={'none'}
